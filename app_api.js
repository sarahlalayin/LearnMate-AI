...
async function loginAsParent() {
  ...
  try {
    ...
    } else { alert('登入失敗'); logout(); }
  } catch(e) { alert('無法連線到伺服器'); logout(); }
}
...
function renderParentInsights(db) {
  ...
  // 為了簡化，直接渲染寫死在 HTML 中的資料，僅動態產生長條圖
  const barData = [
...
function startQuiz(taskId, subject) {
  document.getElementById('s-quiz-subject').textContent = subject;
  activeQuiz = { type: 'daily', id: taskId, questions: MOCK_QUESTIONS, currentIdx: 0 };
...
async function finishQuiz() {
  ...
  try {
    await fetch(`${API_BASE}/tasks/complete`, {
      method: 'POST', headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ familyId: currentFamilyId, taskId: activeQuiz.id, pointsToAdd: earned })
    });
    alert('測驗完成！獲得大量點數！');
    await syncAndRender();
    navTo('screen-student-home');
